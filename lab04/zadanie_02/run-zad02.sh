...
info "WOLUMIN" "Tworzenie woluminu o nazwie $nodejs_volume_name"
docker volume create $nodejs_volume_name

info "KOPIOWANIE" "Kopiowanie zawartości katalogu $nodejs_files_dir do woluminu $nodejs_volume_name"
docker run --rm -v $nodejs_volume_name:/app -v $(pwd):/tmp ubuntu sh -c "cp -r /tmp/$nodejs_files_dir/. /app"

info "KONTENER" "Tworzenie i uruchamianie kontenera Docker z Node.js z zamontowanym woluminem $nodejs_volume_name"
...
info "WERYFIKACJA" "Sprawdzanie czy wolimen $nginx_volume_name istnieje"
...
info "KONTENER" "Tworzenie i uruchamianie kontenera Docker z Nginx i z zamontowanym woluminem $nginx_volume_name"
...
info "WOLUMIN" "Tworzenie woluminu o nazwie $all_volumes_name"
docker volume create $all_volumes_name

info "KOPIOWANIE" "Kopiowanie zawartości woluminów $nginx_volume_name i $nodejs_volume_name do woluminu $all_volumes_name"
